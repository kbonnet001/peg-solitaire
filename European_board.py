...
class European_board:
  def __init__(self, screen_width):
    ...
    self._pegs_position = self._create_pegs_position()
  ...
  def _create_new_game_pegs(self):  # Private
      ...
      pegs = [Peg(peg_pos) for peg_pos in self._pegs_position]
  ...
  def draw(self, screen) : 
    # Dessiner la grille
    for row in range(len(self._pegs_position)):
        for col in range(len(self._pegs_position)):
            if self._pegs_position[row][col] == "*" : 
